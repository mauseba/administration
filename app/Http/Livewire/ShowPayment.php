...
class ShowPayment extends Component
{
        ...
        switch ($this->option) {
            case 'name':
                $query->join('userbs','userbs.id','=','payments.userb_id')
                    ->select('payment_id','Prenom','Nom','userbs.status','amount','statup','date','payments.created_at','payments.updated_at','userbs.id','groupe','hourap')
    ...
    public function createInform()
    {
        ...
        switch ($this->option) {
            case 'name':
                $query->join('userbs','userbs.id','=','payments.userb_id')
                    ->select('payment_id','Prenom','Nom','userbs.status','amount','statup','date','payments.created_at','payments.updated_at','groupe','hourap')
                    ->where('userbs.status', 1)
                    ->where(
                        function($q) {
                          return $q
